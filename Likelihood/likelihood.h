...
//=============================================================================================================================================
	...
	template<typename Event, typename Weighter>
	struct ps_StackingLikelihood{
		private:
			std::vector<double> source_ra;
			std::vector<double> source_dec;
			std::vector<Event> evt_list;
			std::vector<Event>* MC_list;
			double bgNorm;
		public:
			Weighter* psWeighter;
			ps_StackingLikelihood(std::vector<double> source_ra,std::vector<double> source_dec,std::vector<Event> evt_list, std::vector<Event>* MC_list, Weighter* psWeighter):
			source_ra(source_ra),source_dec(source_dec),evt_list(evt_list),MC_list(MC_list),psWeighter(psWeighter){
				assert(source_ra.size()==source_dec.size());
			}

//Initialize precomputes the stacked Kent distribution for each event in the sample. These quantities do not depend on any of the parameters to be optimized (ns,normalization,gamma,E_cut) so it is better to precompute them.
		void Initialize(){
			int num_sources = source_ra.size();
			double pi = boost::math::constants::pi<double>();
			double kappa,cdef,kent,evt_dec,sgSum;
			int loc;
			for(Event& e: evt_list){
//For now we assume that all events have a fixed angular resolution
			 kappa = 1.0/(0.017453*0.017453);
			 evt_dec = e.zenith-pi/2.0;
			 sgSum = 0.0;
			 for(int i=0;i<num_sources;i++){
			   cdef = sin(evt_dec)*sin(source_dec.at(i))+cos(evt_dec)*cos(source_dec.at(i))*cos(source_ra.at(i)-e.azimuth);
			   if (-2.0*kappa<log(1.0e-10)){
			        kent = log(kappa)-log(4.0*pi)-(kappa-log(2.0))+kappa*cdef;
			   }
			   else{
				kent = log(kappa)-log(4.0*pi)-log(sinh(kappa))+kappa*cdef;
			   }
			   sgSum += exp(kent);
			 }
			 e.cachedSourceSum = sgSum/float(num_sources);
			}
			bgNorm = 0.0;
			for(Event& e: (*MC_list)){
			  bgNorm += (*psWeighter).eval_AtmWeight(e);
			}

		}

		double evaluate(std::vector<double> x) const{
			int sample_size=evt_list.size(),loc;
			double pi = boost::math::constants::pi<double>();
		        double bgSum,sgSum,f,llh=0.0;
			double ns = x.at(0);
			double gamma = x.at(1);
			double sgpdf,bgpdf;

			(*psWeighter).setPowerLawIndex(gamma);

		        for(const Event& e: evt_list){
//			   sgSum = log(e.cachedSourceSum)+log((*psWeighter).eval_AstroWeight(e))-log((*psWeighter).eval_BinnedAstroNorm(cos(e.zenith)))-log((*psWeighter).getNormalization());
			   sgSum = log(e.cachedSourceSum);
			   sgpdf = exp(log((*psWeighter).eval_AstroWeight(e))-log((*psWeighter).eval_AstroNorm())-log((*psWeighter).getNormalization()));
			   bgpdf = (*psWeighter).eval_AtmWeight(e)/bgNorm;
//			   sgSum = exp(sgSum+log(sgpdf)-log(sgpdf+bgpdf));
//			   bgSum = 1./(2.2*pi)*exp(log(bgpdf)-log(sgpdf+bgpdf));
			   sgSum = exp(sgSum+log(sgpdf));
			   bgSum = 1./(2.2*pi)*bgpdf;
//			   std::cout << bgSum << "  " << (*psWeighter).eval_AstroWeight(e)/((*psWeighter).eval_AstroNorm()*1.0e-8) << std::endl;
			   f = ns/double(sample_size);
			   if ((sgSum==0.0 && bgSum==0.0)) continue;
    			   llh += log(f*sgSum*1.0e8 + (1.0-f)*bgSum*1.0e8)-log(1.0e8);
		  	}
		        return llh;
		}
	};
